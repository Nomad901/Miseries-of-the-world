...
void Rifle::makeUsualRifle(SDL_Renderer* pRenderer, SDL_FRect pCharRect)
{
	...
	Weapon::setPaths(std::filesystem::current_path() / "Assets" / "photos and ttf" / "rifle.png",
					 std::filesystem::current_path() / "Assets" / "photos and ttf" / "brokenRifle.png");
	Weapon::setShootPath(std::filesystem::current_path() / "Assets" / "photos and ttf" / "shootRifle.png",
						{ {SideOfChar::RIGHT, {0,1,2}} }, 50, 300);
	Weapon::setReloadPath(std::filesystem::current_path() / "Assets" / "photos and ttf" / "reloadRifleV2.png", 
...
void Rifle::makeSniperRifle(SDL_Renderer* pRenderer, SDL_FRect pCharRect)
{
	initRifle(pRenderer, mRifleType, pCharRect,
			  ...
			  Gun::Config::BulletsConfig{ 500, std::filesystem::current_path() / "Assets" / "photos and ttf" / "bulletV2.png", 1000,
			  							  10,3,30,30,2,getWeaponStats().mPos },
			  Gun::Config::WeaponConfig{ 100,25, std::make_pair(8, 12),100,100, getWeaponStats().mPos });
	Weapon::setPaths(std::filesystem::current_path() / "Assets" / "photos and ttf" / "sniperRifle.png",
					 std::filesystem::current_path() / "Assets" / "photos and ttf" / "brokenSniper.png");
	Weapon::setShootPath(std::filesystem::current_path() / "Assets" / "photos and ttf" / "sniperShoot.png", 
						 { {SideOfChar::RIGHT, {0,1,2}} }, 50, 300);
	Weapon::setReloadPath(std::filesystem::current_path() / "Assets" / "photos and ttf" / "sniperReload.png", 
